...
class PeekableArrowStreamReaderTests {
    private static final BufferAllocator ALLOCATOR = new RootAllocator(Integer.MAX_VALUE);

    @Test
    void hasNextBatch() throws IOException {
        ByteArrayInputStream inputStream = new ByteArrayInputStream(new byte[] {1, 2, 3, 4, 5});
        PeekableArrowStreamReader reader = new PeekableArrowStreamReader(inputStream, ALLOCATOR);

        assertThat(reader.hasNextBatch()).isTrue();
    }
}
